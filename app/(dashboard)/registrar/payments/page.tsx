...
export default function VerifyPaymentsPage() {
  ...
  async function load() {
    ...
    const { data } = await supabase
      .from("payments")
      .select("id, gcash_reference, reference_number, proof_image, amount, status, payment_method, verified_at, verified_by, request_id, requests(tracking_code, user_id, profiles(full_name))")
      .order("created_at", { ascending: false });
    setPayments((data as unknown as PaymentRow[]) ?? []);

    const pendingGcash = (data as unknown as PaymentRow[] | null)?.filter(
      (p) => p.status === "Pending" && p.payment_method === "gcash"
    ) ?? [];
    const signedUrls = await Promise.all(
      pendingGcash.map(async (p) => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex w-fit items-center gap-1 rounded-full bg-slate-100 p-1">
        {TABS.map((t) => (
          ...
          >
            {t.label}
            <span className="ml-1.5 text-[11px] font-bold text-brand-600">{counts[t.id]}</span>
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state card">
          ...
          <p className="text-sm font-medium text-slate-500">
            {tab === "pending" ? "No pending payments." : `No ${tab} payments.`}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {visible.map((p) => (
            <div key={p.id} className="card space-y-3">
              ...
              {p.payment_method === "gcash" && tab === "pending" && previews[p.id] && (
